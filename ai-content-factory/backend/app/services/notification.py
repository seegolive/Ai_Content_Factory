...
class NotificationService:
    ...
    async def notify_job_complete(self, video_title: str, clips_count: int, user_email: str) -> None:
        ...
        await asyncio.gather(
            ...
            self.send_email(
                ...
                body=f"<h2>Processing Complete</h2><p>Video: <strong>{video_title}</strong></p>"
    ...
    async def notify_job_error(self, video_title: str, error: str, user_email: str) -> None:
        ...
        await asyncio.gather(
            ...
            self.send_email(
                to=user_email,
                subject=f"❌ Processing failed: {video_title}",
                body=f"<h2>Processing Failed</h2><p>Video: <strong>{video_title}</strong></p>"
                     f"<pre>{error}</pre>",
            ),
            return_exceptions=True,
        )
