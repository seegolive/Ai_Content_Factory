...
@celery_app.task(
    bind=True, max_retries=2, name="app.workers.tasks.distribute.distribute_clip"
)
def distribute_clip(
    self,
    clip_id: str,
    ...
    async def _run():
        ...
        try:
            async with TaskSession() as db:
                ...
                resolved_privacy = (
                    clip.publish_settings.get("privacy", privacy)
                    if clip.publish_settings
                    else privacy
                )
                ...
                for platform in platforms:
                    ...
                    if existing.get("status") == "published":
                        logger.info(
                            f"[Distribute] Clip {clip_id} already published to {platform}, skipping"
                        )
                        continue
                    ...
                    if platform == "youtube" and youtube_account_id:
                        try:
                            ...
                            if yt_account.refresh_token:
                                try:
                                    access_token = (
                                        await yt_service.refresh_access_token(
                            ...
                            upload_title = (
                                ps.get("title") or clip.title or "Untitled Clip"
                            )
                            upload_description = (
                                ps.get("description") or clip.description or ""
                            )
                            ...
                            final_updates[platform] = {
                                "status": "published",
                                "video_id": youtube_id,
                            }
                            logger.info(
                                f"[Distribute] Clip {clip_id} published to YouTube: {youtube_id} (privacy={upload_privacy})"
                            )

                        except Exception as e:
                            logger.error(
                                f"[Distribute] YouTube upload failed for {clip_id}: {e}"
                            )
                            final_updates[platform] = {
                                "status": "failed",
                                "error": str(e),
                        ...
                        logger.warning(
                            f"[Distribute] No youtube_account_id for clip {clip_id}, skipping YouTube"
                        )
                        final_updates[platform] = {
                            "status": "pending",
